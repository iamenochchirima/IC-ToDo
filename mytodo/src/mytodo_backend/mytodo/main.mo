...
shared ({ caller = owner }) actor class mytodo() = this {
  ...
  type MintingRequest = Types.MintingRequest;
  ...
  type CapEvent = Types.CapEvent;
  type CapIndefiniteEvent = Types.CapIndefiniteEvent;
  ...
  private stable var registryState : [(TokenIndex, AccountIdentifier)] = [];
  private stable var tokenMetadataState : [(TokenIndex, Metadata)] = [];
  private stable var ownersState : [(AccountIdentifier, [TokenIndex])] = [];

  private stable var tokenListingState : [(TokenIndex, Listing)] = [];
  private stable var tokenSettlementState : [(TokenIndex, Settlement)] = [];
  private stable var paymentsState : [(Principal, [SubAccount])] = [];
  private stable var refundsState : [(Principal, [SubAccount])] = [];
  private stable var transactionState : [Transaction] = [];

  private var _registry : TrieMap.TrieMap<TokenIndex, AccountIdentifier> = TrieMap.fromEntries(registryState.vals(), ExtCore.TokenIndex.equal, ExtCore.TokenIndex.hash);
  private var _tokenMetadata : TrieMap.TrieMap<TokenIndex, Metadata> = TrieMap.fromEntries(tokenMetadataState.vals(), ExtCore.TokenIndex.equal, ExtCore.TokenIndex.hash);
  private var _owners : TrieMap.TrieMap<AccountIdentifier, [TokenIndex]> = TrieMap.fromEntries(ownersState.vals(), AID.equal, AID.hash);

  //For marketplace
  private var _transactionBuffer = Buffer.Buffer<Transaction>(0);
  private var _tokenListing : TrieMap.TrieMap<TokenIndex, Listing> = TrieMap.fromEntries(tokenListingState.vals(), ExtCore.TokenIndex.equal, ExtCore.TokenIndex.hash);
  private var _tokenSettlement : TrieMap.TrieMap<TokenIndex, Settlement> = TrieMap.fromEntries(tokenSettlementState.vals(), ExtCore.TokenIndex.equal, ExtCore.TokenIndex.hash);
  private var _payments : TrieMap.TrieMap<Principal, [SubAccount]> = TrieMap.fromEntries(paymentsState.vals(), Principal.equal, Principal.hash);
  private var _refunds : TrieMap.TrieMap<Principal, [SubAccount]> = TrieMap.fromEntries(refundsState.vals(), Principal.equal, Principal.hash);
  ...
  private stable var _minter : Principal = owner;
  ...
  system func preupgrade() {
    ...
    ownersState := Iter.toArray(_owners.entries());
    tokenListingState := Iter.toArray(_tokenListing.entries());
    tokenSettlementState := Iter.toArray(_tokenSettlement.entries());
    paymentsState := Iter.toArray(_payments.entries());
    refundsState := Iter.toArray(_refunds.entries());
    transactionState := Buffer.toArray(_transactionBuffer);
  ...
  system func postupgrade() {
    ...
    for (x in transactionState.vals()) {
  ...
  public shared query (msg) func extGetTokenAsset(id : TokenIdentifier) : async Result.Result<Asset, Text> {
    let tokenData = _getTokenData(id);
    switch (tokenData) {
      case (?token) {
        let asset = _assets[Nat32.toNat(_blobToNat32(token))];
        let asset_without_chunks : Asset = {
          name = asset.name;
          thumbnail = null;
          payload = {
        ...
        return #ok(asset_without_chunks);
      };
      case (_) {
  ...
  public query func details(token : TokenIdentifier) : async Result.Result<(AccountIdentifier, ?Listing), CommonError> {
    ...
    switch (_getBearer(tokenid)) {
      case (?token_owner) {
        return #ok((token_owner, _tokenListing.get(tokenid)));
      };
      case (_) {
        return #err(#InvalidToken(token));
      };
  ...
  public query func transactions() : async [Transaction] {
    Buffer.toArray(_transactionBuffer);
  };
  ...
  public shared query func toAID(user : User) : async AccountIdentifier {
    ExtCore.User.toAID(user);
  };
  ...
  public query func http_request(request : HttpRequest) : async HttpResponse {
    ...
    //Just show index
    let t_transactions : [Transaction] = Buffer.toArray(_transactionBuffer);
